...
class DividerDelegate : AdapterDelegate<List<UserListItem>>() {
    ...
    override fun isForViewType(items: List<UserListItem>, position: Int): Boolean = items[position] is UserListItem.Divider
    override fun onCreateViewHolder(parent: ViewGroup): RecyclerView.ViewHolder = ViewHolder(parent)

    internal class ViewHolder(viewGroup: ViewGroup) : RecyclerView.ViewHolder(viewGroup.inflate(R.layout.divider_item_layout))
}
